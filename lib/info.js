...
export function getSortedInfoData() {
  // Get file names under /info
  const fileNames = fs.readdirSync(infoDirectory)
  const allInfoData = fileNames.map(fileName => {
    ...
    // Use gray-matter to parse the info metadata section
...
export function getAllInfoIds() {
  ...
  export async function getInfoData(id) {
    const fullPath = path.join(infoDirectory, `${id}.md`)
    const fileContents = fs.readFileSync(fullPath, 'utf8')
